      ...
      <div className="shd-container" style={{ maxWidth: 900 }}>
        ...
        <Section title="Quick start (8 steps)">
          ...
          <ol style={{ paddingLeft: "1.5rem", listStyle: "decimal" }}>
            ...
            <li>
              <strong>Initial Annual Withdrawal — Frozen Baseline.</strong> Your desired standard of living, set once as
              a real (today's-money) figure. The desk converts it to a quarterly request and applies the Guyton-Klinger
              guardrails from there. This figure deliberately stays frozen — you never manually bump it for inflation.
        ...
        <Section title="Risk Simulator — Monte Carlo fan chart">
          ...
          <ul style={{ paddingLeft: "1.25rem", marginTop: "0.4rem" }}>
            <li>
              ...
              are entered once, alongside the Initial Annual Withdrawal — Frozen Baseline, and every pane reads them.
              The simulator shows them read-only with a <strong>Use real pension details</strong> /{" "}
              <strong>Hypothetical</strong> switch — pick Hypothetical to test a different pension here without ever
              changing Pane 1.
            </li>
            <li>
          ...
          <p style={{ marginTop: "0.75rem" }}>
            ...
            every year — Equities (volatile, real growth from the chosen return model) and Cash (deterministic, earns
            the <em>Cash real return</em> slider in real terms). Each year the engine decides which bucket to spend from
            using a <em>defensive draw threshold</em>: in a <strong>good</strong> year it spends from Equities and
            refills the Cash Pot up to its starting size; in a <strong>bad</strong> year it spends from Cash to avoid
            selling shares at a discount. This captures the modest <em>cash drag</em> on the median (cash compounds
            slower than equities) while showing how the buffer lifts the 5th–10th percentile floor when
            sequence-of-returns risk bites early.
          </p>
          <p style={{ marginTop: "0.75rem" }}>
          ...
          <p style={{ marginTop: "0.75rem" }}>
            <strong>Allocation bias slider.</strong> Sits directly under the two override fields and rebalances
            Equities↔Cash while keeping the total pot fixed — the fastest way to see what tilting another 10% into cash
            (or out of it) does to the fan. The free-text overrides above still take priority, and{" "}
          ...
          <p style={{ marginTop: "0.75rem" }}>
            <strong>Yearly vs Quarterly tick.</strong> A toggle in the simulator header switches the engine between two
            modes. <em>Yearly tick</em> is the original engine — one G-K check per year across 10,000 paths.{" "}
            <em>Quarterly tick</em> is a companion mode that steps the same 10,000 paths four times per year and
          ...
          <p style={{ marginTop: "0.75rem" }}>
            <strong>Withdrawal Recorded field.</strong> Pane 1 now carries a "Withdrawal Recorded" money input,
            auto-seeded from the guardrail-adjusted <em>Request</em>. Overwrite it with the actual £ that left the pot
            each quarter; the value is stored on the ledger row and shown in the timeline table so every commit has a
            clean audit trail of what was withdrawn.
          </p>
          <p style={{ marginTop: "0.75rem" }}>
            <strong>Audit Mode (hidden).</strong> Double-click the "Risk Simulator" header (inside the standalone Risk
            Simulator page) to freeze the RNG and run a single deterministic path with canonical inputs (Age 64→85,
            £610k/£90k, £36k draw, £12,700 pension @ 67). A 2-decimal step ledger renders beneath the chart for
            pocket-calculator reproduction. Double-click again to exit.
          </p>
          <p style={{ marginTop: "0.75rem" }}>
        ...
        <Section title="Scenario Stress Test (Pane 2 — a quick in-app preview, not the Risk Simulator)">
          <p>
            Pane 2 has its own small <strong>Scenario Stress Test</strong> slider — a single "what if equities dropped
            X% right now" preview that recalculates the whole directive live, so you can see how the app would react
            before it actually happens. It's entirely local to this page: nothing is saved, and moving the slider back
            to 0% instantly restores your real directive.
          </p>
          <p style={{ marginTop: "0.5rem" }}>
            This is deliberately lightweight — one hypothetical, one number, no probability attached to it. It is{" "}
            <strong>not</strong> the same thing as the Risk Simulator (below), which is a separate, standalone Companion
            App that runs a full Monte Carlo simulation across 10,000 possible market paths and shows the whole
            distribution of outcomes as a fan chart. If you want to sanity-check "what does the directive do in a
            2008-style crash right now", use the Pane 2 slider. If you want "how likely is my entire plan to survive to
            age 95", use the Risk Simulator.
          </p>
        </Section>
        ...
        <Section title="Companion Apps — Compare vs 4% Rule (Historical)">
          <p>
            ...
            main dashboard. The first is <strong>Compare vs 4% Rule (Historical)</strong>, which backtests your plan
            against every real rolling US retirement since 1928, alongside a faithful classic 4% Rule replica — using
            the same Guyton-Klinger engine that drives this app, not a separate approximation.
          </p>
          <p>
